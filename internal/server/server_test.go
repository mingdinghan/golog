...
// testProduceConsume tests that producing and consuming works
// by using client and server to produce a record to the log, consume it back,
// and then check that the record sent is the same one received
...
// testConsumePastBoundary tests that server responds with an api.ErrOffsetOutOfRange() error
