...
// TestPickerNoSubConnAvailable() tests that a picker initially returns `balancer.ErrNoSubConnAvailable` before
// the resolver has discovered servers and updated the picker's state with available sub-connections
...
// TestPickerProducesToLeader() tests that the picker picks the leader sub-connection for append calls
...
// TestPickerConsumesFromFollowers() tests that the picker picks the followers sub-connections in a round-robin for consume calls
...
// setupTest() builds the test picker with some mock sub-connections
